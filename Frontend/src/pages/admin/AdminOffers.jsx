...
const SCOPE_LABELS = {
  global:     { label: 'Global',      cls: 'bg-blue-100 text-blue-700' },
  group:      { label: 'Grupo',       cls: 'bg-amber-100 text-amber-700' },
  individual: { label: 'Individual',  cls: 'bg-violet-100 text-violet-700' },
}
...
export default function AdminOffers() {
  ...
  const handleSave = async (payload) => {
    const data = { ...payload, scope: 'global' }
    if (formOffer?.id) {
      const res = await api.patch(`/offers/${formOffer.id}`, data)
  ...
  return (
    <div className="p-4 md:p-8">
      <div className="flex items-center justify-between mb-6 md:mb-8">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-800">Todas las ofertas</h1>
          <p className="text-slate-500 text-sm mt-1">Gestión global de ofertas de la plataforma</p>
        </div>
        <button
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100">
              ...
              <th className="text-left px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Alcance</th>
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={6} className="px-6 py-16 text-center">
                  <p className="text-slate-400 text-sm">No hay ofertas registradas.</p>
                </td>
              </tr>
            ) : (
              offers.map((offer) => {
                const scope = SCOPE_LABELS[offer.scope] ?? SCOPE_LABELS.individual
                return (
                  <tr key={offer.id} className="hover:bg-violet-50/40 transition-colors">
                    <td className="px-6 py-4">
                      ...
                      {offer.business && <p className="text-xs text-slate-400 mt-0.5">{offer.business.name}</p>}
                      {offer.group    && <p className="text-xs text-slate-400 mt-0.5">{offer.group.name}</p>}
                    </td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${scope.cls}`}>
                        {scope.label}
                      </span>
                    </td>
                    <td className="px-6 py-4">
            ...
            return (
              <div key={offer.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 space-y-3">
                ...
                <div className="flex flex-wrap gap-2 text-xs">
                  <span className={`inline-flex items-center px-2.5 py-1 rounded-full font-semibold ${scope.cls}`}>
                    {scope.label}
                  </span>
